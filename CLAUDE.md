...
There are no build/lint/test commands — this repo has no `package.json` or tooling of any kind. To work on the site, edit the files directly and open `index.html` in a browser (or serve the directory with any static file server) to preview changes.
...
- `styles/style.css` — one global stylesheet, no preprocessor. Theme values (colors, fonts) are defined once as CSS custom properties in `:root` (`--bg-color`, `--primary-color`, `--secondary-color`, `--glass-bg`, `--glass-border`, `--font-main`) — change the theme by editing these rather than hardcoding colors in new rules. Visual style is a dark "glassmorphism" look (`.glass-card`, `.glass-nav` use `backdrop-filter: blur(...)` over translucent backgrounds). CSS is organized in the same top-to-bottom order as the sections in `index.html`, plus a single mobile breakpoint (`max-width: 768px`) at the bottom.
